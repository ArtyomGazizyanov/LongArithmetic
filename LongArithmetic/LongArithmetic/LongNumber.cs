...
{//класс длинных чисел
    class LongNumber
    {
        ...
        public LongNumber(IEnumerable<byte> bytes)
        {
            _discharges.AddRange(bytes);
        }
        public LongNumber(IEnumerable<byte> bytes, bool isNegative)
        {
            _discharges.AddRange(bytes);
        ...
        public override string ToString()
        {
            ...
            sb.Append(_isNegative?"-":"+");
            sb.Append(" ");
            for (int i = _discharges.Count-1; i >=0; i--)
                sb.Append(_discharges[i]).Append("-");

            sb.Remove(sb.Length - 1, 1);
            return sb.ToString();
        }
        // поменять знак
        public void ChandgeSing()
        {
            _isNegative = !_isNegative;
        }
        // поменять конкретную позицию
        public void ChandgeInPosition(byte b, int i)
        {
            _discharges[i] = b;
        }
        // получить из позиции
        public byte GetfromPosition( int i)
        {
            return _discharges[i];
        }
        // получить символ знака
        public char Sign()
        {
            return _isNegative?'-':'+';
        }
